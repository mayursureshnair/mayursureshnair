<!-- README_optionA.md -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=glitch&color=gradient&custom_color_list=0:1f0036,30:3b0764,70:581c87,100:6b21a8&height=220&section=header&text=MAYUR%20SURESH%20NAIR&fontSize=46&fontColor=ffffff" width="100%" alt="Glitch Header" />

  <br />

  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=19&pause=1000&color=C084FC&center=true&vCenter=true&width=600&lines=Building+Foundations+in+AWS%2C+GCP+%26+Azure;Exploring+AI+Agents%2C+Generative+AI+%26+Edge+Tech;Linux+Systems+Admin+%7C+Bash+%7C+Python+%7C+Cloud-Native" width="100%" alt="Typing SVG Animation" />

  <br /><br />
</div>

---

## 💻 About Me

I am a **BCA Student at Manipal University Jaipur** with a strong passion for **Cloud Computing, Artificial Intelligence, and Systems Infrastructure**.

I am currently building my core engineering foundation through intensive hands‑on learning—exploring cloud architecture, edge computing technologies, security fundamentals, and AI while continually strengthening my knowledge of Linux, Python, Bash scripting, and modern distributed systems.

- 📍 **Location**: Kannur, Kerala, India  
- ☁️ **Cloud Computing**: Hands‑on experience with **AWS** (primary focus), **GCP** (secondary), and **Azure** (AZ‑900 certified).  
- 🤖 **Artificial Intelligence & Agents**: Exploring generative AI, AI agent workflows, and practical Python AI implementations.  
- 🐧 **Linux & Systems**: Deepening knowledge in Linux administration, command‑line automation, shell scripting, and containerization.  
- 🎯 **Product Engineering Mindset**: Focused on industry best practices, security fundamentals, and scalable technology solutions.  
- 🤝 **Open To**: Peer collaboration, open‑source contributions, hackathons, and technical learning opportunities.

---

## 🛠️ Tech Stack & Ecosystem

<div align="center">
  <img src="https://skillicons.dev/icons?i=py,bash,postgres,aws,gcp,azure,docker,kubernetes,linux,git,github,vscode&perline=6" width="100%" alt="Skill Icons" />
</div>

<br />

- 🐍 **Languages**: Python, Bash shell scripting, SQL  
- ☁️ **Cloud Platforms**: AWS (primary focus), Google Cloud Platform (GCP), Microsoft Azure (AZ‑900)  
- 🐧 **Systems & DevOps**: Linux administration, Git, GitHub, Docker, Kubernetes  
- 🤖 **AI & Developer Tools**: Google Antigravity, Google ADK, VS Code  

---

## ⚡ AI & Cloud Domain Focus

| Domain                              | Focus Level | Technical Details & Learning Roadmap |
|------------------------------------|:-----------:|--------------------------------------|
| **Cloud Architecture & Infrastructure** | `Primary`   | Hands‑on AWS multi‑service deployment, GCP fundamentals, Azure infrastructure configurations. |
| **Linux & Systems Administration** | `Core`      | Bash automation, security management, kernel basics, CLI mastery. |
| **Containerization & Orchestration** | `Practitioner` | Docker containerization, Kubernetes clusters, CI/CD pipelines. |
| **AI & Generative AI Agents**      | `Active Focus` | Python‑based AI agents, Vibe Coding workflows, LLM prompt engineering, edge AI integration. |
| **Version Control & Security**     | `Core`      | Git branching, GitHub Actions, cloud IAM, open‑source best practices. |

---

## 🎓 Education & Academic Journey

### **Bachelor of Computer Applications (BCA)** | *Manipal University Jaipur*  
`2026 – 2029 (Expected)`

- **Specialization**: Computer Applications, Cloud Infrastructure, Artificial Intelligence, Systems Programming  
- **Core Technical Focus**: Operating Systems, DBMS, Data Structures, Computer Networks, Python Systems  
- **Mission**: Fuse university fundamentals with industry‑grade cloud and AI certifications to craft production‑ready technology skills.

---

## 🎖️ Verified Credentials & Developer Profiles

<div align="center">
  <table align="center" border="0">
    <tr>
      <td align="center" width="200">
        <a href="https://www.credly.com/users/mayursureshnair/badges/credly">
          <img src="https://img.icons8.com/color/48/credly.png" width="24" height="24" alt="Credly" valign="middle" />
          <br /><b>Credly Profile</b>
        </a>
      </td>
      <td align="center" width="200">
        <a href="https://www.skills.google/public_profiles/a3c06da5-61e4-41b4-b08a-dfe3de8c809e">
          <img src="https://img.icons8.com/color/48/google-cloud.png" width="24" height="24" alt="Google Skills" valign="middle" />
          <br /><b>Google Skills</b>
        </a>
      </td>
    </tr>
    <tr>
      <td align="center" width="200">
        <a href="https://me.developers.google.com/u/mayursureshnair">
          <img src="https://img.icons8.com/color/48/google-logo.png" width="24" height="24" alt="Google Developers" valign="middle" />
          <br /><b>Google Developers</b>
        </a>
      </td>
      <td align="center" width="200">
        <a href="https://www.credential.net/profile/mayursureshnair/wallet">
          <img src="https://img.icons8.com/color/48/warranty.png" width="24" height="24" alt="Accredible" valign="middle" />
          <br /><b>Accredible Wallet</b>
        </a>
      </td>
    </tr>
  </table>
</div>

---

## 📬 Connect & Network

<div align="center">
  <table align="center" border="0">
    <tr>
      <td align="center" width="200">
        <a href="https://www.linkedin.com/in/mayursureshnair/">
          <img src="https://img.icons8.com/color/48/linkedin.png" width="24" height="24" alt="LinkedIn" valign="middle" />
          <br /><b>LinkedIn</b>
        </a>
      </td>
      <td align="center" width="200">
        <a href="mailto:mayursureshnair@gmail.com">
          <img src="https://img.icons8.com/color/48/gmail-new.png" width="24" height="24" alt="Gmail" valign="middle" />
          <br /><b>Gmail</b>
        </a>
      </td>
    </tr>
  </table>
</div>

---

<div align="center">
  <br />
  <p><i>"Artificial Intelligence does not replace human ingenuity; it extends human potential across computing boundaries."</i></p>
  <br />

  <img src="https://capsule-render.vercel.app/api?type=wave&color=gradient&custom_color_list=0:1f0036,30:3b0764,70:581c87,100:6b21a8&height=120&section=footer&fontSize=0" width="100%" alt="Wave Footer" />
</div>
